...
const translations = {
  en: {
    ...
    section3_content: <p><strong>/simple_simulate</strong><br>
      ...
      Full simulation with detailed 5★ and 4★ stats, including distribution by type (limited vs. standard), guaranteed drops, distances between 5★, and base 4★ stats. Optimal for getting a complete picture.</p>
  ...
  ja: {
    ...
    section1_content: <ul>
      <li><strong>ガチャ回数</strong> — 1回のセッションでのガチャ回数（最大1000回）。</li>
    ...
    section2_title: "ソフトピティとは何か、それはどのように機能するのか？",
  ...
  zh: {
    ...
    section1_content: <ul>
      <li><strong>抽卡次数</strong> — 每个模拟会话中的抽卡次数（最多1000次）。</li>
      <li><strong>模拟次数</strong> — 为统计而重复进行抽卡会话的次数（最多100,000次）。</li>
  ...
  es: {
    ...
    section1_content: <ul>
      <li><strong>Número de tiradas</strong> — cantidad de tiradas en una sesión (máximo 1000).</li>
  ...
  pt: {
    ...
    section1_content: <ul>
      <li><strong>Número de giros</strong> — quantidade de giros em uma sessão (máximo 1000).</li>
    ...
    section2_content: <p>Na maioria dos jogos gacha existe um sistema de soft pity. Seus parâmetros exatos geralmente são ocultos, mas o bot usa uma fórmula aproximada. O soft pity começa no 65º giro, depois disso a chance de obter 5★ aumenta seguindo uma curva em S e chega a 100% no 80º giro.</p>
      <p><strong>Fórmula:</strong><br>
      Probabilidade = base + (1 - base) × [1 / (1 + e^(-12 × (x - 0.5)))]<br>
      onde x = (pity - 65) / 15</p>,
  ...
  fr: {
    ...
    section1_content: <ul>
      <li><strong>Nombre de tours</strong> — nombre de tours par session (maximum 1000).</li>
      ...
      <li><strong>Type de bannière</strong> — choix entre 50/50 (chance de 50 % d’obtenir un héros limité) ou un héros limité garanti.</li>
